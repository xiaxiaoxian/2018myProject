...
<script>
    ...
    export default{
        ...
        computed: {
            ...
            checkedCount(){ // 如果选中的数量和数组的长度一致：全选
                let i = 0;
            ...
            totalPrice(){
                ...
                this.cartList.forEach((item)=>{
                    if(item.checked == '1'){
                        money += parseFloat(item.productPrice)*parseFloat(item.productNum);
                    }
                });
                return money;
            }
        ...
        methods:{
            init(){
                axios.get('/users/cartList').then((response)=>{ // 请求数据
            ...
            editCart(flag,item){  // 编辑购物车
                ...
                axios.post('/users/cartEdit',{  // 传递到服务器端
