...
<script>
    ...
    export default{
	    ...
	    computed:{ // 方法2:vuex自带方法
		    
		    ...mapState(['nickName','cartCount']) // ...es6数组结构运算
//		    nickName:{
//		        get(){
//		        	return this.$store.state.nickName; // 组件需要使用vuex中保存的nickName值
//		        },
//			    set(){
//
//			    }
...
//		    }
        ...
        methods:{
            checkLogin(){ // 检查是否登录：刷新页面状态不变
	            axios.get('/users/checkLogin').then((response)=>{
                	let res = response.data;
                    if(res.status == "0"){
                       // me.nickName = res.result;
	                    this.$store.commit('updateUserInfo',res.result); // 通过store中的commit提交mutations
	                    this.getCartCount();
            ...
            login(){ // 登录
                if(!this.userName || !this.userPwd){ // 校验用户名和密码不为空
                    this.errorTip = true;
                    return;
                }
                axios.post('/users/login',{ // 登录
                ...
                axios.post('/users/logout').then((response)=>{
                    ...
                    if( res.status == "0" ){
                    	 this.$nextTick(function(){
                    	 	 this.nickName = "";
		                     this.$store.commit('updateUserInfo',''); // 清除vuex中存放的值
	                    })
                    }
                })
            },
